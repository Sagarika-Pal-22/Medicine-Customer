...
import myrehabcare.in.databinding.ActivityMyDoctorsBinding;
import okhttp3.Call;
...
public class MyDoctorsActivity extends AppCompatActivity implements MyDoctorsAdapter.ItemClickListener{

    private ActivityMyDoctorsBinding binding;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        binding = ActivityMyDoctorsBinding.inflate(getLayoutInflater());
    ...
    private void setList(){
        ...
        jsb.post("http://mrcadmin.in/api/doctorlist", body, new Callback() {
            @Override
                    ...
                    activity.runOnUiThread(new Runnable() {
                        ...
                        public void run() {
                            try {
                                ...
                                if (!jsonObject.getString("message").toLowerCase().replace(" ","").contains("doctorfound")){
                                ...
                                for(int i = 0; i < jsonArray.length(); i++) {
                                    ...
                                    String aa = objects.getString("rating");
                                    float a = 0;
                                    try {
                                    ...
                                    doctors.setRating(a);
                                    doctors.setService_type(objects.getString("service_type"));
                                    doctors.setUploadPrescription(objects.getString("upload_prescription"));
                                    doctors.setType(objects.getString("role")+":"+objects.getString("is_fav"));
    ...
    @Override
    public void onItemClick(View view, int position,Doctors doctors) {
        startActivity(new Intent(this, DrProfileActivity.class).putExtra("doctors", doctors));
    }
}